...
province_rollup <- ps_results_long_out %>% group_by(province_code, province_name_eng) %>%
  summarize(
    ...
    ps_confirmed_closed = length(ps_code[ps_open == "NO"]),
    ps_status_unknown = length(ps_code[ps_open == "UNKNOWN"]),
    ps_audited = length(ps_code[audit_or_recount == "YES"]),
    ps_invalidated = length(ps_code[ps_invalidated == "YES" & !is.na(ps_invalidated)]),
    ...
    ps_reporting_zero_votes = length(ps_code[reporting_zero_votes == "YES" & !is.na(reporting_zero_votes)]),
    ...
    invalidated_ps_votes = sum(invalidated_results_sheet_total, na.rm = T),
...
province_rollup <- province_rollup %>% 
  ...
  rename(
    ...
    turnout = total_preliminary_votes / total_vr) %>% 
  dplyr::select(
    province_code, province_name_eng, ps_planned, total_vr, 
    ps_reporting, ps_confirmed_closed, ps_status_unknown, 
    ps_audited, ps_invalidated, missing_bvv_ps_reporting, ps_reporting_zero_votes, 
    ps_reporting_excess_turnout, ps_added_votes_in_audit,
    invalidated_ps_votes, pre_deduplication_bvv_vote_total, post_deduplication_bvv_vote_total, known_bvv_votes_under_audit, known_net_change_from_audit,
